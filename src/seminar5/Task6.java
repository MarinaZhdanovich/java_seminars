...
class HeapSort { //Куча представляет собой специальное дерево, где каждый элемент удовлетворяет свойству кучи:
    // для каждого узла i его дочерние узлы имеют значения, которые меньше (в случае мин-кучи)
    // или больше (в случае макс-кучи) значения узла i.
    public static void buildTree(int[] tree, int sortLength) {
        // Начнем строить сортирующее дерево с середины массива и идем в обратном порядке
        for (int i = sortLength / 2 - 1; i >= 0; i--) {
            heapify(tree, sortLength, i);
        }
    }

    public static void heapify(int[] tree, int sortLength, int root) {
        int largest = root; // Индекс наибольшего элемента считаем корневым элементом
        int leftChild = 2 * root + 1; // Индекс левого дочернего элемента
        int rightChild = 2 * root + 2; // Индекс правого дочернего элемента

        // Если левый дочерний элемент существует и больше корневого элемента
        if (leftChild < sortLength && tree[leftChild] > tree[largest]) {
            largest = leftChild;
        }

        // Если правый дочерний элемент существует и больше корневого элемента
        if (rightChild < sortLength && tree[rightChild] > tree[largest]) {
            largest = rightChild;
        }

        // Если наибольший элемент не корневой, меняем их местами и рекурсивно вызываем heapify
        if (largest != root) {
            int swap = tree[root];
    ...
    public static void heapSort(int[] sortArray, int sortLength) {
        ...
        // Постепенно уменьшаем размер сортирующего дерева и меняем корневой элемент с последним элементом массива
        for (int i = sortLength - 1; i > 0; i--) {
            int temp = sortArray[0];
            sortArray[0] = sortArray[i];
            sortArray[i] = temp;

            // Вызываем heapify на уменьшенном дереве
